...
- [x] Application launches via `uv run python -m ink`
- [x] Application launches via `python src/ink/main.py`
- [x] Main window displays on screen
- [x] Application runs without errors in console
- [x] Application exits cleanly when window is closed
- [x] Exit code is 0 for successful runs

### 4.2 Startup Performance
- [x] Application launches in < 2 seconds on typical hardware
- [x] No visible delay between command and window appearance
- [x] Window appears in responsive state (not "not responding")

### 4.3 Logging Requirements
- [x] Startup log message printed to console
- [x] Main window display log message printed
- [x] Exit log message with exit code printed
- [x] Log level is INFO by default
- [x] Log format includes timestamp, module, level, message

### 4.4 Error Handling
- [x] Missing Qt libraries produce clear error message
- [x] Import errors display module name and suggestion
- [x] Uncaught exceptions logged before exit
- [x] Window close doesn't leave zombie processes

### 4.5 Testing
- [x] Integration test: Application starts and exits successfully
- [x] Integration test: Exit code is 0 on normal exit
- [x] Manual test: Launch and close application
- [x] Manual test: Application shows in taskbar/window manager
