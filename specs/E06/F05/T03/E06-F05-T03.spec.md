...
- **Priority**: P0 (MVP)
- **Status**: Draft
- **Parent**: [E06-F05](../E06-F05.spec.md)
- **Created**: 2025-12-26
- **Estimated Hours**: 3
- **Actual Hours**:
- **Effort**: Low
- **Tags**: [ui, actions, menus, panels]
...
- State syncs bidirectionally (menu ↔ panel visibility)
...
- [ ] Hierarchy panel toggle action in Panels menu
- [ ] Properties panel toggle action in Panels menu
- [ ] Messages panel toggle action in Panels menu
- [ ] Reset Panel Layout action in Panels menu
- [ ] Separator before Reset Layout action for visual grouping

### 4.2 Action Behavior
- [ ] Clicking action toggles panel visibility
- [ ] Keyboard shortcuts work from any widget with focus
...
- [ ] Action state syncs when panel closed via X button
- [ ] Action state syncs when panel visibility changed programmatically
...
- [ ] Shortcuts displayed in menu next to action text
- [ ] No shortcut conflicts with other actions

### 4.4 Visual Polish
- [ ] Tooltips appear on hover with descriptive text
- [ ] Status bar shows shortcut hint when action hovered
- [ ] Menu labels use mnemonic underscores (&Hierarchy)
- [ ] Actions disabled if panels not initialized (defensive)

### 4.5 Testing
- [ ] Unit tests for action creation and configuration
- [ ] Test shortcut bindings are correctly assigned
- [ ] Test action checkmark sync with panel visibility
- [ ] Test raise behavior when showing hidden panel
- [ ] UI test: click action, verify panel toggles
- [ ] UI test: press shortcut, verify panel toggles
- [ ] 80%+ code coverage on action-related code
