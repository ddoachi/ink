...
- **Priority**: P0 (MVP)
- **Status**: Draft
- **Parent**: [E06-F06](../E06-F06.spec.md)
- **Created**: 2025-12-26
- **Estimated Hours**: 3
- **Actual Hours**:
- **Effort**: Small
- **Tags**: [infrastructure, persistence, migration]
...
- [ ] Settings version tracking implemented
- [ ] Version stored in settings on first run
- [ ] Migration framework with sequential migration support
- [ ] `reset_all_settings()` clears all settings and re-initializes defaults
- [ ] `reset_window_geometry()` clears only window geometry
- [ ] `reset_recent_files()` clears recent files list
- [ ] Settings file path accessible via `get_settings_file_path()`
- [ ] `get_all_settings()` returns dictionary of all settings
- [ ] `export_settings()` exports to JSON file
- [ ] `is_corrupted()` detects corrupted settings
- [ ] Reset actions in Help > Settings menu
- [ ] Reset confirms with user before executing
- [ ] Reset restarts application
- [ ] Corrupted settings detected at startup with recovery option
- [ ] Future migration examples documented
