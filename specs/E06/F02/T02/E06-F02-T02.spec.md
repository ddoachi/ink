...
- [x] File > Open shows file dialog with `.ckt` and `.cdl` filters
- [x] Selected file path triggers `_load_netlist()` method
- [x] Recent Files submenu shows up to 10 most recent files
- [x] Most recently opened file appears at top of Recent Files list
- [x] Clicking recent file opens it directly
- [x] Missing recent files show warning dialog and are removed from list
- [x] "Clear Recent Files" removes all entries
- [x] Exit action (`Ctrl+Q`) closes application
- [x] Keyboard shortcuts work: `Ctrl+O` for Open, `Ctrl+Q` for Exit
- [x] Recent files persist across application restarts
- [x] File paths in Recent Files menu show full path
- [ ] Status bar shows "Loaded: {file}" message on successful load (deferred to E06-F05)
