...
- **Actual Hours**:
- **Effort**: Medium
- **Tags**: [domain-model, aggregate, design]
...
- [ ] `Design` class implemented as aggregate root
- [ ] Collections for cells, nets, pins, ports with private storage
- [ ] `add_cell()`, `add_net()`, `add_pin()`, `add_port()` methods with duplicate checking
- [ ] ID-based getters: `get_cell()`, `get_net()`, `get_pin()`, `get_port()` (O(1))
- [ ] Name-based getters: `get_cell_by_name()`, `get_net_by_name()`, `get_port_by_name()` (O(1))
- [ ] Collection accessors: `get_all_cells()`, `get_all_nets()`, `get_all_pins()`, `get_all_ports()`
- [ ] Statistics methods: `cell_count()`, `net_count()`, `pin_count()`, `port_count()`, `sequential_cell_count()`
- [ ] `validate()` method to check referential integrity
- [ ] ValueError raised for duplicate IDs or names
- [ ] Name indexes maintained for O(1) lookup
- [ ] Immutable collection views returned (not internal dicts)
- [ ] Unit tests with 95%+ coverage
- [ ] Test coverage includes: add operations, duplicate detection, getters, validation
