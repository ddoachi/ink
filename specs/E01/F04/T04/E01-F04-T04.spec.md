...
- [ ] `TopologyAnalyzer` protocol defined
- [ ] `TransistorTopologyAnalyzer` implements protocol
- [ ] `TransistorPatternRecognizer` identifies transistor structures
- [ ] `SignalFlowGraph` dataclass with connections and identified structures

### 4.2 Inverter Recognition
- [ ] Detects complementary PMOS/NMOS pairs as inverters
- [ ] Correctly identifies input (gate) and output (drain) nodes
- [ ] Handles different VDD/VSS naming conventions
- [ ] Ignores transistors not forming valid inverter structure

### 4.3 Transmission Gate Recognition
- [ ] Detects PMOS/NMOS pairs with shared source/drain
- [ ] Marks signal flow as bidirectional
- [ ] Identifies control nodes (CLK, CLK_B)
- [ ] Distinguishes from inverters (no VDD/VSS connection)

### 4.4 NAND/NOR Recognition
- [ ] Detects parallel PMOS + series NMOS as NAND
- [ ] Detects series PMOS + parallel NMOS as NOR
- [ ] Handles multi-input gates (NAND3, NOR4, etc.)
- [ ] Correctly maps all inputs to output

### 4.5 Hierarchical Analysis
- [ ] `register_cell_pinout()` accepts cell pin definitions
- [ ] Analyzes subcircuit instances using registered pinouts
- [ ] Gracefully handles unknown cell types (skip with warning)

### 4.6 Integration
- [ ] Output format compatible with E01-F04-T02 `register_subcircuit_topology()`
- [ ] Works with CDL Parser output format

### 4.7 Testing
- [ ] Unit tests for inverter pattern recognition
- [ ] Unit tests for transmission gate recognition
- [ ] Unit tests for NAND/NOR recognition
- [ ] Unit tests for D-latch analysis (SR latch structure)
- [ ] Unit tests for D-FF analysis (master-slave structure)
- [ ] Integration test with E01-F04-T02
