...
- [x] `Cell` dataclass has `is_sequential: bool` field with default `False`
- [x] Graph construction calls `latch_identifier.is_sequential()` for each cell
- [x] `is_sequential` flag stored in graph node attributes
- [x] `is_sequential_cell(cell_name)` query method implemented
- [x] `get_sequential_cells()` returns list of sequential cells
- [x] `get_sequential_count()` returns count of sequential cells (as `sequential_cell_count()`)
- [x] Query methods handle missing `is_sequential` attribute gracefully (default `False`)
- [x] Existing unit tests updated to handle new `Cell` field
- [x] Integration test validates sequential cells tagged correctly from sample netlist
- [x] 90%+ test coverage on query methods
